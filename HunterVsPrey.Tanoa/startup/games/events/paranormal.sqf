	...
	if (vehicle player isEqualTo player) then {
		...
		if (_eventRoll isEqualTo 0) then {
			if (side player isEqualTo WEST || side player isEqualTo RESISTANCE) then {
				_event = execVM "startup\games\events\paranormal\1ghost.sqf";
				waitUntil {scriptDone _event};
			};
			if (side player isEqualTo EAST) then {
				_event = execVM "startup\games\events\paranormal\1teleport.sqf";
				waitUntil {scriptDone _event};
			};
		};
		if (_eventRoll isEqualTo 1) then {
			if (side player isEqualTo WEST || side player isEqualTo RESISTANCE) then {
				_event = execVM "startup\games\events\paranormal\2decoy.sqf";
				waitUntil {scriptDone _event};
			};
			if (side player isEqualTo EAST) then {
				_event = execVM "startup\games\events\paranormal\2darkness.sqf";
				waitUntil {scriptDone _event};
			};
		};
		...
		if (_eventRoll isEqualTo 3) then {
			_event = execVM "startup\games\events\paranormal\ghostchat.sqf";
		};
	};
};
