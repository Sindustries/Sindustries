...
*/
	...
	waitUntil {_uav distance _uavArea <= _uavScanSize || (damage _uav) >= 0.7};	
	...
	["HUDuavLayer","HVPHUDuavImg"] remoteExec ["HVP_fn_showEventIcon", 0];
	
	for "_timer" from 0 to _uavTime do {
		while {(damage _uav) < 1 && _uavTime > 0  && _uav distance _uavArea <= _uavScanSize} do {
			_playerCount = {isPlayer _x && alive _x && _x distance _uavArea <= _uavScanSize} count allUnits;
		
			if (_playerCount > 0) then {
				{
					if (isPlayer _x && alive _x && _x distance _uavArea <= _uavScanSize) then {
						...
						[_x,_aMarker,_uav,_uavArea,_uavScanSize,_uavUpdate] spawn {
							...
							while {(damage _uav) < 0.7 && _uav distance _uavArea <= _uavScanSize && _trackedPlayer distance _uavArea <= _uavScanSize && alive _trackedPlayer} do {
								_aMarker setMarkerPos (getPos _trackedPlayer);
								sleep _uavUpdate;
							};
							if ((damage _uav) > 0.7 || _uav distance _uavArea > _uavScanSize || _trackedPlayer distance _uavArea > _uavScanSize || !alive _trackedPlayer) exitWith {
	...
	_uav setDamage 1;
	["HUDuavLayer"] remoteExec ["HVP_fn_hideEventIcon", 0];