...
if (HVPGameType isEqualTo 1) then {
	if (isServer) then {
						...
						while {!_spawnFound} do {
							...
							if (_posCheck && _distCheck) then {
								...
								if ((random 100) < 50) then {
									...
									[_x, 10] remoteExec ["SMS_fn_setUnconscious", _x];
								};
					...
					if (isPlayer _x && alive _x) then {
						...
						while {!_spawnFound} do {
							...
							if (_posCheck && _distCheck) then {
								_spawnFound = true;
								_usedSpawnPos pushBack _spawnPos;
								[_spawnPos] remoteExec ["HVP_fn_haloSpawn", _x];
		...
		if (HVPSpawnType isEqualTo 3) then {
			[WEST] spawn HVP_fn_heliSpawn;
			[EAST] spawn HVP_fn_heliSpawn;
			if (_mercAlive > 0) then {
				[RESISTANCE] spawn HVP_fn_heliSpawn;
			};
		};
		if (HVPSpawnType isEqualTo 4) then {
			[WEST] spawn HVP_fn_podSpawn;
			[EAST] spawn HVP_fn_podSpawn;
			if (_mercAlive > 0) then {
				[RESISTANCE] spawn HVP_fn_podSpawn;
			};
		};
	};
};
		...
		if (isPlayer _x && alive _x) then {
			if (HVPSpawnType isEqualTo 0) then {
				...
				if (_spawn isEqualTo "Heli") then {
					[(side _x)] remoteExec ["HVP_fn_heliSpawn", _x];
				};
				if (_spawn isEqualTo "Pod") then {
					[(side _x)] remoteExec ["HVP_fn_podSpawn", _x];
				};
			};
			if (HVPSpawnType isEqualTo 1) then {
				_spawnFound = false;
