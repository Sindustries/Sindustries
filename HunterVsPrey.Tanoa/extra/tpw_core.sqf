...
tpw_mods_version = "20160827"; // will appear on start hint
...
tpw_core_fnc_civs =
	...
	if (_ethnicity == "european") then 
		{
		_civstring = ["c_man","c_man","c_man","RDS"] select _ext;
	...
	if (_ethnicity == "african") then 
		{
		_civstring = ["c_man","caf_ag_afr_civ","lop_afr_civ_man","c_man"] select _ext;
		[_civstring] call tpw_core_fnc_grabciv;
	...
	if (_ethnicity == "mideast") then 
		{
		_civstring = ["c_man","caf_ag_me_civ","lop_tak_civ_man","c_man"] select _ext;
		[_civstring] call tpw_core_fnc_grabciv;
